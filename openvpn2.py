...
def wait_for_vpn_connection(management_ip, management_port, timeout=60):
    ...
    while time.time() - start_time < timeout:
        try:
            with socket.create_connection((management_ip, management_port), timeout=2) as sock:
...
def main():
    ...
    # 启动OpenVPN客户端
    openvpn_process = subprocess.Popen([openvpn_executable, "--config", config_file_path, "--management", management_ip, str(management_port)])
